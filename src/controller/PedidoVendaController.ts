...
export class PedidoVendaController extends PedidoVenda {
    ...
    static async atualiza (req: Request, res: Response): Promise<Response> {
        try {
            // Extrai os dados atualizados do corpo da requisição
            const pedidoAtualizado: PedidoVendaDTO = req.body;
            ...
            if (isNaN(idPedido)) {
                return res.status(400).json({ mensagem: "ID do pedido inválido. Verifique e tente novamente." });
            }
    
            // Chama o método de atualização no modelo PedidoVenda
            const respostaModelo = new PedidoVenda(
                pedidoAtualizado.idCliente,
                pedidoAtualizado.idCarro,
                pedidoAtualizado.dataPedido,
                pedidoAtualizado.valorPedido
            );
    
            // Verifica se a atualização foi bem-sucedida
            if (respostaModelo) {
                return res.status(200).json({ mensagem: "Pedido de venda atualizado com sucesso!" });
            } else {
                return res.status(400).json({ mensagem: "Não foi possível atualizar o pedido. Verifique os dados ou entre em contato com o administrador do sistema." });
            }
        } catch (error) {
            console.log(`Erro ao atualizar o pedido de venda: ${error}`);
